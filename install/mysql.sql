...
CREATE TABLE `AuditVoting` (
  `UserID` varchar(50) NOT NULL DEFAULT '',
  `ItemID` varchar(50) NOT NULL,
  `VoteType` enum('N','Y', 'L') NOT NULL DEFAULT 'N',
  `ModificationDate` double NOT NULL DEFAULT '0',
...
CREATE TABLE `NodeProperties` (
  ...
  KEY `NodeID` (`NodeID`),
  CONSTRAINT `NodeProperty_ibfk_2` FOREIGN KEY (`NodeID`) REFERENCES `Node` (`NodeID`) ON DELETE CASCADE
...
CREATE TABLE `VotingLemons` (
 ...
 PRIMARY KEY (`VoteID`), 
  ...
  KEY `IssueID` (`IssueID`), 
  KEY `NodeID` (`NodeID`), 
  CONSTRAINT `VotingLemons_ibfk_1` FOREIGN KEY (`UserID`) REFERENCES `Users` (`UserID`) ON DELETE CASCADE, 
  CONSTRAINT `VotingLemons_ibfk_2` FOREIGN KEY (`IssueID`) REFERENCES `Node` (`NodeID`) ON DELETE CASCADE, 
  CONSTRAINT `VotingLemons_ibfk_3` FOREIGN KEY (`NodeID`) REFERENCES `Node` (`NodeID`) ON DELETE CASCADE 
) ENGINE=InnoDB DEFAULT CHARSET=utf8 ROW_FORMAT=DYNAMIC;                                                                                                  
